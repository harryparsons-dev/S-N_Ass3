		...
		public void run() {
			...
			try {
				...
				try {
					...
					System.out.println("Recieved: " + byteArrayToHexString(message3ct));
					...
					byte[] encryptedClientNonceInc = encAEScipher
					...
					System.out.println("sending: " + byteArrayToHexString(message4body));
					if (debug)
					...
					if (serverNonce + 1 != serverNonceReply) {
						if (debug)
							System.out.println("Nonces dont match");
					...
					if (debug)
						System.out.println("Secret sent: " + new String(message6pt));
					myConnection.close();
